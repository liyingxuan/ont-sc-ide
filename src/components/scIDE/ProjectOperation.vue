<template>
  <div class="pro-operation-page">
    <div class="row">
      ...
      <div class="col pro-operation-btn-center" v-show="isShowCheck">
        <button class="btn btn-outline-dark pro-operation-button" @click="getCheckPage" :class="[showCheck ? 'pro-operation-button-click' : '']" data-toggle="tooltip" data-placement="bottom" :title="$t('projectOperation.check')">{{$t('projectOperation.check')}}</button>
      </div>
    ...
    <div class="pro-operation-border">
      ...
      <div v-show="showCheck" class="pro-operation-height">
        <Check :isShowPreDeployAndPreRun='isShowPreDeployAndPreRun'></Check>
      </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      isShowCheck() {
        return this.projectInfo.info.language === 'Python' ? true : false
      }
    },
    methods: {
      showCompilePage(){
      ...
      getCheckPage(){
        this.$router.push({ name:'IDE-Check',params:{projectName:this.projectName.info.projectName}})
      },
      getDeployPage(){
      ...
      getPage(){
        ...
        }else if(this.$route.name === 'IDE-Check'){
          this.showCheckPage()
