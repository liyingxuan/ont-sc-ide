  ...
  <div class="container-fluid ide" >
    ...
    <div class="row ide-content">
      ...
      <div class="ide-ide col-lg-8 col-md-8 col-sm-8 col-xs-12">
        <project-ide v-on:showPreDeployAndPreRun="isShowPreDeployAndPreRun"></project-ide>
      </div>
  ...
  export default {
    ...
    mounted(){
      if(this.ProjectName.info.id) {
        let title = (LangStorage.getLang('zh') === "zh") ? zh.ide.waiting : en.ide.waiting
        let content = (LangStorage.getLang('zh') === "zh") ? zh.ide.waitingInfo : en.ide.waitingInfo
    ...
    methods: {
      saveProject(){
        ...
        this.$store.dispatch('saveProject', param).then(res => {
          let title = (LangStorage.getLang('zh') === "zh") ? zh.ide.saveSuccess : en.ide.saveSuccess
          let content = (LangStorage.getLang('zh') === "zh") ? zh.ide.saveSuccessContent : en.ide.saveSuccessContent
          let payload = {
            title:title,
            content:content,
