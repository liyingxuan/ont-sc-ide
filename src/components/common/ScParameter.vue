    ...
    <div class="run-input-and-txt" >
        <select v-model="parameter.type" style="margin-right: 5px;height:30px;" @change="changeParameterTypeTip(parameter)">
            ...
            <span v-if="parameter.type === 'Boolean'" class="run-input-radio">
                <input type="radio" value="true" v-model="parameter.value[parameter.type]" id="trueValue" :checked="parameter.value">
                <label for="trueValue">True</label>
                <input type="radio" value="false" v-model="parameter.value[parameter.type]" id="falseValue" :checked="parameter.value">
            ...
            <div v-else-if="parameter.type === 'Array'" class="card-param-input">
                ...
                <textarea name="" id=""  rows="10" :class="arrayParamControl?'expand-textarea': 'collapse-textarea' " v-model="parameter.value[parameter.type]" ></textarea>
             ...
             <div v-else-if="parameter.type === 'Map'" class="card-param-input">
                ...
                <textarea name="" id=""  rows="10" :class="mapParamControl?'expand-textarea': 'collapse-textarea' " v-model="parameter.value[parameter.type]" ></textarea>
                <a-icon type="question-circle-o" class="textarea-control-icon" @click="showMapTip" />
             </div>
             <div v-else class="card-param-input">
                <input class="run-input" v-model="parameter.value[parameter.type]"  :placeholder="parameter.typeTip" >
             </div>
...
export default {
    ...
    methods: {
        ...
        handleConfirmParam(param) {
            ...
            this.parameter.value[this.parameter.type] = param
        },
        handleCancel() {
            this.showJsonEditor = false
        }
